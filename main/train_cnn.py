...
tf.app.flags.DEFINE_string("confusion_matrix_dir",get_config_values('model','cnn_confusion_matrix'),"path of confusion_matrix")
...
tf.app.flags.DEFINE_integer("decay_step", 7000, "how many steps before decay learning rate.")
tf.app.flags.DEFINE_float("decay_rate", 0.15, "Rate of decay for learning rate.")
            ...
            for epoch in range(curr_epoch,FLAGS.num_epochs):
                ...
                for batch in tqdm(train_data):
                    ...
                    if iteration % 3690 == 0:
                        ...
                        if (epoch == FLAGS.num_epochs-2) or (epoch == FLAGS.num_epochs-1):
                            print ("Model %d\tEpoch %d\tConfusion matrix:" % (K, epoch))
                            pprint (confusion_matrix)
                            save_pickle(FLAGS.confusion_matrix_dir+'confusion_matrix-model{0}-epoch{1}'.format(K,epoch), confusion_matrix)
                #epoch increment
